...
class Cli_Memcache {
	...
	public function __invoke( $args, $assoc_args ) {
		...
		if ( 'enable' === $args[0] ) {
			...
			if ( empty( $port ) ) {
				return \WP_CLI::error( 'SG Optimizer was unable to connect to the Memcached server and it was disabled. Please, check your SiteGround control panel and turn it on if disabled.' );
			}
